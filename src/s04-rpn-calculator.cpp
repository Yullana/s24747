...
auto evaluate_degree(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for **"};
    }
...
auto evaluate_module(std::stack<double>& stack) -> void
{
    if (stack.size() < 1) {
        throw std::logic_error{"not enough operands for md"};
    }
    auto const a = pop_top(stack);
    stack.push(abs(a));
}
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_degree(stack);
            } 
            else if (each == "sqrt") {
                evaluate_square_root(stack);
            } 
            else if (each == "md") {
                evaluate_module(stack);
            } 
            else {
